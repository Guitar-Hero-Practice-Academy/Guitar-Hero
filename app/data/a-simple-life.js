window.initialSongs.push({
  id: "john-farnham-a-simple-life",
  contentRevision: 1,
  title: "A Simple Life",
  artist: "John Farnham",
  album: "Romeo's Heart",
  albumYear: 1996,
  albumOrder: 1,
  difficulty: "Intermediate",
  tuning: "E A D G B E",
  key: "A",
  bpm: "",
  videoUrl: "",
  chords: [
    { name: "A", frets: "x02220", fingers: "123" },
    { name: "Asus4", frets: "x02230", fingers: "12 3" },
    { name: "F#m", frets: "244222", fingers: "34" },
    { name: "E", frets: "022100", fingers: "231" },
    { name: "D", frets: "xx0232", fingers: "132" },
    { name: "G", frets: "320003", fingers: "21 3" },
    { name: "Bm", frets: "x24432", fingers: "342" },
    { name: "Bm/A", frets: "x04432", fingers: "3421" },
    { name: "C#m", frets: "x46654", fingers: "342" },
    { name: "D/B", frets: "x20232", fingers: "3 2" }
  ],
  strumming: [
    {
      name: "Main ballad strum",
      bpm: "",
      beats: ["D", "", "D", "U", "", "U", "D", "U"],
      accents: [0, 4],
      chordRun: "A  Asus4  A  /  F#m  E  D  E",
      note: "Keep it steady and let the chord changes breathe."
    }
  ],
  chart: `[Intro]
A  Asus4 A / F#m E D E
[Verse 1]
A          D
Show me the magic
A         E
I must've lost it somewhere
A    D       A       E
In my dreams, so far away
A            E
I turned around, everything's changing but
G
Where is the soul in my song?
[Chorus 1]
Asus4 A
Once there was heaven in my eyes
Asus4  F#m    E      D
Whatever happened to those skies?
Bm
Somebody show me how to
Bm/A G     D    A
Get back to the simple life
[Interlude 1]
A  Asus4   A  / F#m E D  E
[Verse 2]
A         D

A            E
Temptation's all around me
The train came along and I got on board
A                 D
I danced with the darkness
A              E
The skies were the limit
G
It's such a long way to fall
[Chorus 2]
A     Asus4 A
Once there was heaven in my eyes
Asus4 F#m    E       D
Whatever happened to those skies?
Bm
Somebody show me how to
Bm/A G             A
Get back to the simple life
E            A    Asus4 A
'cause I've done my time in paradise
Asus4         F#m     E       D
I've tried it all maybe once or twice
Bm
Somebody show me how to
Bm/A G     D      A
Get back to the simple life
[Bridge]
C#m          D/B
Stars keep me going
And the moon is there to light my way
C#m                   F#m
I need to go where my heart is free
G
Looking for the way back, hoping for the way back
[Chorus 3]
E A Asus4 A Asus4 F#m E D
(Instrumental lead-in over chorus chords)
Bm
Somebody show me how to
Bm/A  G     D     A
Get back to the simple life
E           A    Asus4 A
'cause I've done my time in paradise
Asus4        F#m    E       D
I've tried it all maybe once or twice
Bm                   Bm/A
Somehow the grass was always greener

G     D    A
On the other side
E                     Asus4 A
Once there was heaven in my eyes
Asus4   F#m     E       D
Whatever happened to those skies?
Bm
Somebody show me how to
Bm/A  G    D     A
Get back to the simple life
Asus4        F#m      E      D
I've tried it all maybe once or twice
Bm          Bm/A G
Somebody show me show show show me
D     A
A simple life
E                     Asus4 A
Once there was heaven in my eyes
Asus4                       F#m
Once there was heaven in my eyes
E                                 Asus4
Somebody show me, show me... (Fade out)`
});
