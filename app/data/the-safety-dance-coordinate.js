const safetyDanceCoordinate = window.initialSongs.find((song) => song.id === "men-without-hats-the-safety-dance");

if (safetyDanceCoordinate) {
  safetyDanceCoordinate.contentRevision = 2;
  safetyDanceCoordinate.chart = `[Intro riff - capo 1]
e|--------------------|
B|--------------------|
G|--------------------|
D|--------------------|
A|-5-5--5------5--5-|
E|-------8-7-8------|

[Verse 1]
B
We can dance if we want to
       A                  E
We can leave your friends behind
           A
Cause your friends don't dance and if they don't dance
             B             F#
Well they're no friends of mine
     B
I say, we can go where we want to
              A               E
A place where they will never find
           A
And we can act like we come from out of this world
          B            F#
Leave the real one far behind
           E          A   D  G
(x2)
And we can dance  (Dansez!)
B
We can go when we want to
             A               E
The night is young and so am I
           A
And we can dress real neat from our hats to our feet
       B                          F#
And surprise 'em with the victory cry
     B
Say, we can act if want to
      A            E
If we don't nobody will
            A
And you can act real rude and totally removed
          B               F#
And I can act like an imbecile

[Chorus]
       E             A
I say, we can dance, we can dance
D                       G
Everything's out of control
E             A
We can dance, we can dance
D                           G
We're doing it from pole to pole's
E             A
We can dance, we can dance
D                      G
Everybody look at your hands
E             A
We can dance, we can dance
D                    G
Everybody takin' the cha-A-A-ance
B F#                   C# G#
    It's safe to dance,    Oh well it's safe to dance,
B F#                      C# G#
    Yes it's safe to dance

[Verse 2]
B
We can dance if we want to
          A                 E
We've got all your life and mine
    A
As long as we abuse it, never gonna lose it
B                      F#
Everything'll work out right
     B
I say, we can dance if we want to
       A                    E
We can leave your friends behind
            A
'Cause your friends don't dance and if they don't dance
             B             F#
Well they're no friends of mine

[Chorus]
       E             A
I say, we can dance, we can dance
D                       G
Everything's out of control
E             A
We can dance, we can dance
D                           G
We're doing it from pole to pole
E             A
We can dance, we can dance
D                      G
Everybody look at your hands
E             A
We can dance, we can dance
D                    G
Everybody takin' the cha-A-A-ance
B F#                   C# G#
   It's safe to dance,    Oh well it's safe to dance,
B F#                      C# G#
    Yes it's safe to dance
B F#                       C# G#
    Well it's safe to dance     Well it's safe to dance
    Yes it's safe to dance     Well it's safe to dance
    Well it's safe to dance    It's a Safety Dance
    Well it's a Safety Dance    Oh it's a Safety Dance
    Oh it's a Safety Dance      Well it's a Safety Dance`;
}
