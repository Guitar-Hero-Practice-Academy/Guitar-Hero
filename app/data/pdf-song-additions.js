window.initialSongs.push(
  {
    id: "nofx-the-irrationality-of-rationality",
    contentRevision: 1,
    title: "The Irrationality of Rationality",
    artist: "NOFX",
    album: "The War on Errorism",
    albumYear: 2003,
    albumOrder: 3,
    difficulty: "Beginner",
    tuning: "E A D G B E",
    key: "Am",
    bpm: "",
    videoUrl: "",
    sourceImages: [
      { src: "assets/the-irrationality-of-rationality-chart-1.png", caption: "Full PDF chart - page 1" },
      { src: "assets/the-irrationality-of-rationality-chart-2.png", caption: "Full PDF chart - page 2" }
    ],
    chords: [
      { name: "Am", frets: "x02210", fingers: "231" },
      { name: "G", frets: "320003", fingers: "21 3" },
      { name: "F", frets: "133211", fingers: "134211" },
      { name: "C", frets: "x32010", fingers: "32 1" }
    ],
    strumming: [
      {
        name: "Main punk-folk pulse",
        bpm: "",
        beats: ["D", "", "D", "U", "", "U", "D", "U"],
        accents: [0, 4],
        chordRun: "Am  G F G",
        note: "Keep the Am riff tight, then punch the G-F-G tag cleanly."
      }
    ],
    chart: `[Simplified Am riff]
e|--0-0-0-0-0-0-0-0-0----------------|
B|--1-1-1-1-1-1-1-1-1----------------|
G|--2-2-2-2-2-2-2-2-2----------------|
D|--2-2-2-2-2-2-2-2-2------3---------|
A|--0-0-0-0-0-0-0-0-0----------------|
E|---------------------------3---3----|

[Intro]
Am  G F G  (x5)

[Verse]
Am  C  F  F
Am  C  F  F
Am  G F G
Am  G F G

[Chorus]
C  Am  F  C
C  Am  F  C
C  G   F  C
C  G   Am  G F G
Am  G F G  (x3)

[Verse]
Am  C  F  F
Am  C  F  F
Am  G F G
Am  G F G

[Chorus]
C  Am  F  C
C  Am  F  C
C  G   F  C
C  G   Am  G F G  Am G G C

[Bridge]
Am  C  F  F

[Verse]
Am  C  F  F
Am  C  F  F

[Chorus]
C  Am  F  C
C  Am  F  C
C  G   F  C
C  G   F

[Outro]
G F G  Am Am`
  },
  {
    id: "alanis-morissette-all-i-really-want",
    contentRevision: 1,
    title: "All I Really Want",
    artist: "Alanis Morissette",
    album: "Jagged Little Pill",
    albumYear: 1995,
    albumOrder: 1,
    difficulty: "Beginner",
    tuning: "E A D G B E",
    capo: "1st fret",
    key: "A",
    bpm: "",
    videoUrl: "",
    sourceImages: [
      { src: "assets/all-i-really-want-chart-1.png", caption: "Full PDF chart - page 1" },
      { src: "assets/all-i-really-want-chart-2.png", caption: "Full PDF chart - page 2" },
      { src: "assets/all-i-really-want-chart-3.png", caption: "Full PDF chart - page 3" },
      { src: "assets/all-i-really-want-chart-4.png", caption: "Full PDF chart - page 4" }
    ],
    chords: [
      { name: "A5", frets: "x022xx", fingers: "1" },
      { name: "G5", frets: "3x00xx", fingers: "13" },
      { name: "D5", frets: "xx032x", fingers: "1" },
      { name: "A", frets: "x0222x", fingers: "123" },
      { name: "D5*", frets: "xx0235", fingers: "1234" },
      { name: "C", frets: "x32013", fingers: "32 14" },
      { name: "D", frets: "xx0232", fingers: "132" }
    ],
    strumming: [
      {
        name: "Main riff",
        bpm: "",
        beats: ["D", "", "D", "U", "", "U", "D", ""],
        accents: [0, 4],
        chordRun: "A5  G5  D5",
        note: "Let the A5 sit, then snap into the G5-D5 tag."
      }
    ],
    chart: `[Intro]
A5

[Main riff]
A5  G5  D5

[Verse]
A5
G5  D5
A5
G5  D5

[Pre-chorus]
A5
G5  D5
A5
G5  D5

[Chorus]
A
G5  D5
A
G5  D5

[Verse 2]
A5
G5  D5
A
G5  D5

[Pre-chorus 2]
A5
G5  D5
A5
G5  D5

[Chorus 2]
A
G5  D5
A
G5  D5

[Interlude]
A  G5  D5  (x3)

[Bridge]
C  D
C  D
C  D
A

[Verse 3]
A5
N.C.
A5
G5  D5

[Pre-chorus 3]
A5
G5  D5
A5
G5  D5

[Chorus 3]
A
G5  D5
A
G5  D5
A
G5  D5
A
G5  D5

[Outro]
A
G5  D5
A
(fade)`
  },
  {
    id: "lumineers-cleopatra-acoustic",
    contentRevision: 1,
    title: "Cleopatra",
    artist: "The Lumineers",
    album: "Cleopatra",
    albumYear: 2016,
    albumOrder: 1,
    difficulty: "Absolute beginner",
    tuning: "E A D G B E",
    capo: "1st fret",
    key: "G",
    bpm: 150,
    videoUrl: "https://www.youtube.com/watch?v=tQ1zMvP8teE",
    sourceImages: [
      { src: "assets/cleopatra-chart-1.png", caption: "Full PDF chart - page 1" },
      { src: "assets/cleopatra-chart-2.png", caption: "Full PDF chart - page 2" },
      { src: "assets/cleopatra-chart-3.png", caption: "Full PDF chart - page 3" }
    ],
    chords: [
      { name: "G", frets: "320003", fingers: "21 3" },
      { name: "D", frets: "xx0232", fingers: "132" },
      { name: "C", frets: "x32010", fingers: "32 1" },
      { name: "Em", frets: "022000", fingers: "12" }
    ],
    strumming: [
      {
        name: "Whole song",
        bpm: 150,
        beats: ["D", "U", "D", "U", "D", "U", "D", "U"],
        accents: [0, 4],
        chordRun: "G  D  C  G",
        note: "Straight eighth-note down-up motion; keep it light and even."
      }
    ],
    chart: `[Intro]
G

[Verse]
G
D
G
D

[Break]
G

[Pre-chorus]
G  C  G
D
G  C  G
C  G  D

[Chorus]
G  D  C  G  C  G  D
G  D  C  G  C  G  D

[Break]
G

[Pre-chorus 2]
G  C  G
D
G  C  G
C  G  D

[Chorus]
G  D  C  G  C  G  D
G  D  C  G  C  G  D

[Bridge]
Em  G  C  D  Em
G  Em  C  D  G

[Break]
G

[Pre-chorus 3]
G  C  G
D
G  C  G
C  G  D

[Chorus]
G  D  C  G  C  G  D
G  D  C  G  C  G  D

[Ending]
D  G`
  }
);
